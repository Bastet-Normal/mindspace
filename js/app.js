...
const app = {
    ...
    renderInsights() {
        ...
        // Calculate Inner Climate Index Score (Sunny=100, Breezy=85, Cloudy=60, Foggy=50, Rainy=35, Stormy=15)
        const weightedSum = (counts.sunny * 100) + (counts.breezy * 85) + (counts.cloudy * 60) + (counts.foggy * 50) + (counts.rainy * 35) + (counts.stormy * 15);
        const score = totalCount > 0 ? Math.round(weightedSum / totalCount) : null;
        ...
        if (score !== null) {
            ...
            gaugeTip.innerHTML = '记录你的内心天气与随笔，获取专属的心灵晴空指数。';
        }
    ...
    bindEvents() {
        ...
        document.getElementById('save-log-btn').addEventListener('click', async () => {
            ...
            this.showModal('已记入心空', '您的心灵气象已保存在本地，接纳心空的起伏与风雨。', [
                ...
                { text: '好的', class: 'primary-btn', onClick: () => {
                    this.hideModal();
        ...
        document.getElementById('btn-clear-all').addEventListener('click', () => {
            this.showModal('危险操作！擦除本地数据', '确定要清除所有的本地气象与随笔记录吗？这将完全抹掉设备上的所有数据，且不可逆。', [
                { text: '点错了', class: 'secondary-btn', onClick: () => this.hideModal() },
