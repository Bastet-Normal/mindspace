...
- 📱 **可安装应用**：保留网页版，同时支持手机 PWA、Android APK，以及 Windows / Linux 桌面端打包。
...
### 软件版下载
正式安装包会发布在 [GitHub Releases](https://github.com/wangjiehu/mindspace/releases)。下载对应系统与架构即可：
- Android：下载 `Android-debug.apk` 可直接侧载安装；`Android-release-unsigned.aab` 用于后续正式签名或应用商店发布。
- Windows ARM64：适合 Surface Pro X、Windows on ARM 等设备。
- Windows x64：适合常见 Intel / AMD Windows 电脑。
- Linux x64：提供 AppImage 与 deb 安装包。
...
### Android 原生壳
Android 端使用 Capacitor 包裹同一套静态网页应用，保留账号登录、正念呼吸、内心天气、随笔、专注钟和数据管理能力。
...
本机编译 Android 需要 Java 与 Android SDK；仓库内置 GitHub Actions 会在 Linux runner 上自动构建 Android APK/AAB，并在打 `v*` 标签时上传到 Release。
...
> 体积说明：Electron 兼容性最好，但每个桌面包都会携带 Chromium。Android 端已采用 Capacitor + 系统 WebView，避免把桌面 Chromium 打进手机包。
...
├── favicon.ico               # 浏览器与桌面壳共用图标入口
├── manifest.webmanifest      # PWA 安装清单
├── sw.js                     # 离线缓存 Service Worker
├── assets/                   # PWA、桌面端与安装包图标资源
├── capacitor.config.json     # Android/Capacitor 应用配置
├── android/                  # Android 原生壳工程
├── desktop/
│   └── main.cjs              # Electron 桌面端主进程
├── scripts/
│   ├── prepare-mobile-web.cjs # 生成 Android 内置静态资源
│   ├── apply-android-icons.cjs # 生成 Android 多密度图标
│   └── run-gradle.cjs        # 跨平台 Gradle 启动脚本
├── tests/
...
测试覆盖 Chromium、Firefox、WebKit、Pixel 7 Android Chrome、Galaxy S8 窄屏 Android Chrome 和 iPhone 14 Safari 移动视口，验证页面加载、启动登录门、导航、情绪记录、随笔列表、气象导图、气象历、呼吸/专注/设置入口与 PWA 元数据。GitHub Actions 还会构建 Windows、Linux 与 Android 安装包。
