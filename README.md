...
在数字喧嚣的当下，MindSpace 拒绝繁复的数据对比、指标打分与社交干扰。我们采用极简的毛玻璃美学设计，辅以本地离线播放的高品质白噪音和正念呼吸引导，在本地存储层为您构建起一座纯净、安宁、完全属于您个人的**心灵避风港**。

---

## 🎨 极简淡雅美学 (Aesthetic Design Philosophy)
MindSpace v1.0.0 贯彻极度“轻量”、“淡雅”与“无压感”的设计语言：
* **统一的镂空轻感交互 (Outlined UI)**：所有控制按钮（包括设置操作按钮、弹出框内的操作按钮）均舍弃了厚重的纯色填充，采用精细的镂空边框（Outlined）设计，消除了大块色彩给视觉带来的压迫感。
* **温润的悬停微光反馈**：鼠标移入按钮时，仅以极低的 `8%` 透明主题色进行缓慢的渐变填充，配合微小的悬浮位移与柔和阴影，带来细腻的交互确认感。
* **高对比度原生字体**：采用平滑美观的原生系统无衬线字体，搭配高度适中的字距与行高，带来绝佳的离线阅读体验。
* **隐形极细滚动条**：自定义超薄半透明滚动条，在不活动时近乎隐形，净化阅读视野。
...
* **本地化高保真音轨**：内置 6 种完全本地化、支持离线播放的自然原声白噪音：
  * 🌧️ **柔和雨声** · 🍃 **穿林微风** · 🌊 **潮起潮落**
  * 🔥 **林中篝火** · 🐦 **晨曦鸟鸣** · ☕ **午后咖啡馆**
* **多通道淡入淡出**：开启、关闭、切换音轨时声音自动平滑过渡，避免瞬间噪音骤变刺激耳膜。
...
├── sw.js                      # Service Worker (实现完全离线运行及音频本地缓存)
├── assets/                    # 图标资源及本地音频
